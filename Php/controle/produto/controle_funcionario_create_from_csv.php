...
while (($linhaArquivo = fgetcsv($ponteiroArquivo, 1000, ";")) !== false) {
    ...
    foreach ($funcionario as $cargo) {
        if ($cargo->getNomeCargo() == $linhaArquivo[4]) {
            $cargoExistente = true;
        }
    }
...
$resposta->cargosCadastrados = $funcionario;
$resposta->totalCargos = $qtdCargos;
$resposta->totalFuncionarios = $qtdFuncionarios;
$resposta->FuncionariosCadastrados = $funcionarios;
